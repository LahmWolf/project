...
class Admin_Controllers_Index extends Libs_Controller{
	...
	public function loctheothang(){
		$thang = isset($_REQUEST['thang']) ? $_REQUEST['thang'] : "";
		if($thang==0){
			echo "Chọn giá trị tháng!!!!";
		}
		if($thang != 0){
			$con = new PDO("mysql:host=localhost;dbname=watch;charset=UTF8","root","");
			    $query = "SELECT * FROM donhang WHERE Month(created)='".$thang."'";
	...
	public function sumHD(){
		$ma = isset($_REQUEST['ma']) ? $_REQUEST['ma'] : "";
			if ($ma != "") {
			    $con = new PDO("mysql:host=localhost;dbname=watch;charset=UTF8","root","");
			    $query = "SELECT thanhtien FROM donhang WHERE trangThai='Đã giao' AND  Month(created)='".$ma."' ";
	...
	public function sumHD2(){
		$ngay = isset($_REQUEST['ngay']) ? $_REQUEST['ngay'] : "";
			if ($ngay != "") {
			    $con = new PDO("mysql:host=localhost;dbname=watch;charset=UTF8","root","");
			    $query = "SELECT thanhtien FROM donhang WHERE DATE(created)='".$ngay."'";
	...
	public function sumKH(){
		...
		echo '<span style="color:red;font-size:30px;">Tổng số khách hàng đăng ký vào hệ thống:  '.$result.'</span>' ;
	}
